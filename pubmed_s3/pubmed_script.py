...
def process_file(filename):
    ...
    try:
        ...
        print(filename[:-4])
        df = retrieve_metadata(str(data_lines), filename[:-4])#GET DOI FOR THE PAPER
...
if __name__ == '__main__':
    ...
    # create a list of filenames in the input directory
    filenames = os.listdir(input_dir)
    
    # create a pool of worker processes
    with Pool(num_processes) as pool:
        # map the process_file function to the filenames list
        # the tqdm function is used to display a progress bar
        # for the processing of the files
        list(tqdm(pool.imap(process_file, filenames), total=len(filenames)))
...
for filename in tqdm(os.listdir(input_dir)):
    ...
    try:
        ...
        if total_edges.shape[0] > 0: #remove after testing
            ...
            output_filename = f"{filename[:-4]}.csv" # strip ".txt" and add "_processed.csv"
            output_filepath_nodes = os.path.join(output_dir, f"df_nodes_{output_filename}")
            output_filepath_edges = os.path.join(output_dir, f"df_edges_{output_filename}")
            total_nodes.to_csv(output_filepath_nodes, index=False)
            total_edges.to_csv(output_filepath_edges, index=False)
